...
func main() {
	...
	// Open serial port
	serialPort, err := serial.OpenPort(&serial.Config{Name: port, Baud: baud, ReadTimeout: time.Millisecond * 100})
	if err != nil {
		log.Fatalln(err)
	...
	// duty := []byte{0x00, 0x00}
	// period := []byte{0x01, 0x0A}
	// beep_times := 0
	// buzzer, err := cgo.GetBuzzerBlink(beep_times, 1, period, duty)
	// if err != nil {
	// 	log.Println(err)
	// 	return
	// }
	buzzer, err := cgo.ResetPosStatus()
	if err != nil {
		log.Println(err)
		return
	}
	...
	// buzzer, err := cgo.SetBuzzerStatus(0x01)
	// if err != nil {
	// 	log.Println(err)
	// 	return
	// }
	...
	// t00 := time.Now()
	pon, err := cgo.PowerOnContactless(time.Millisecond * 100)
	if err != nil {
		log.Println(err)
		return
	}
	...
	//query cmdid
	for {
		if c := cmdid(); c == 0 || c == cgo.CMD_BUSY || c == cgo.CMD_CONTINUE {
			fmt.Printf("while cmdid: 0x%X\n", c)
			if out, err := cgo.PackQueryLatestCmdResult(); err != nil {
				log.Printf("error packing query cmdid: %v", err)
				return
	...
	data2, err := nfc.GetAllDataNFC()
	if err != nil {
		log.Printf("error reading from nfc: %v", err)
		return
	}

	log.Printf("Data NFC Uid: %X\n", data2)

	/**/

	/**/

	tick := time.NewTicker(time.Millisecond * 3000)
